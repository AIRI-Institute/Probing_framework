...
class TransformersLoader:
    ...
    def get_token_ids(self, ud_tokenization: List[str], model_tokenization: List[str], word_indices: List[int]
        ...
        def get_model_token():
            token = model_tokenization[ind_model]
            if token.startswith("##"):
                return token[2:]
            return token

        for ind in range(len(ud_tokenization)):
            mapping.append([])
            char_num = 0
            while char_num < len(ud_tokenization[ind]):
                model_token = get_model_token().lower()
                if ud_tokenization[ind][char_num:char_num+len(model_token)].lower() == model_token:
                    char_num += len(model_token)
                    ind_model += 1
