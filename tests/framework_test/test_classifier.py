...
class TestClassifier(unittest.TestCase):
    ...
    def test_MDLLinearModel(self):
        model = MDLLinearModel(input_dim=self.input_dim, num_classes=self.num_classes)
    ...
    def test_LinearVariational(self):
        model = LinearVariational(
            in_features=self.input_dim, out_features=self.num_classes, parent=KL
        )

        mu = torch.nn.Parameter(
            torch.FloatTensor(self.input_dim, self.num_classes).normal_(
                mean=0, std=0.001
            )
        )
