...
class TestTextFormer(unittest.TestCase):
    ...
    samples_reference = {
        "tr": np.array(
            [
                ("Район традиционно развивается как сельскохозяйственный .", "0"),
                ("Он посвящен лицам года в американской политике .", "1"),
                (
                    "Ведомства проверяют торговые операции банка по покупке и продаже валюты .",
                    "2",
                ),
            ]
        ),
        "te": np.array(
            [
                ("Ангел стал известен как опытный духовник .", "0"),
    ...
    def test_samples_dict(self):
        task_name = "test_sent_len"
        data = TextFormer(probe_task=task_name, data_path=self.path_testfile)
        samples, unique_labels = data.form_data()
