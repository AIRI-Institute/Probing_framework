...
class TestProbingConlluFilter(unittest.TestCase):
    ...
    def test__filter_conllu_all_found(self):
        ...
        self.assertEqual(self.probing_filter._filter_conllu("by_passive")[0], by_passive_res)
        self.assertEqual(
            self.probing_filter._filter_conllu("SOmatchingNumber")[0], SOmatchingNumber_res
        )
        self.assertEqual(
    ...
    def test_filter_and_convert_all_saved(self):
        queries_sents = {
            'ADVCL': [
                ...
                'Как защитить их , себя казня , до жуткой храня поры ?', 'Как сообщается в пресс - релизе университета , программу можно использовать на любом смартфоне .',
        ...
        self.probing_filter.upload_files(dir_conllu_path=self.dir_conllu_path)
        self.probing_filter.filter_and_convert(queries=self.queries,
    ...
    def test_filter_and_convert_too_few_sentences(self):
        self.probing_filter.upload_files(dir_conllu_path='./test_filter_probing_data/conllu_folder/recl_too_few')
        with self.assertRaises(Exception):
            self.probing_filter.filter_and_convert(queries=self.queries,
                                                   task_name='cl')
    ...
    def test_filter_and_convert_wrong_partitions_sum(self):
        ...
        with self.assertLogs() as captured:
            self.probing_filter.filter_and_convert(queries=self.queries, partition=partition)
